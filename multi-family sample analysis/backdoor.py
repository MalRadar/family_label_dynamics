...
def yml_merge(rider_name, host_name, dst):
    ...
    try:
        ...
        with open(rider_name, 'r') as rf:
            r = rf.readlines()
            riders = [f.rstrip() for f in r]
        with open(host_name, 'r') as hf:
            h = hf.readlines()
            hosts = [f.rstrip() for f in h]
...
def main(rider, host, output_folder, tmp_folder):
    ...
    ************************************************************************************
    ...
    apk_cmd.repackage(target_apk, os.path.join(new_folder, 'code'))
    apk_cmd.sign_apk(target_apk, 'my.keystore', 'weaken', version='old', password='weaken')
    # if not os.path.exists(target_apk):
    #     with open('error-output.txt', 'a') as f:
    #         f.write(os.path.basename(host)[:-4]+'_'+os.path.basename(rider)[:-4]+'.apk\n')
...
if __name__ == '__main__':
    ...
    main(sys.argv[1].strip(), sys.argv[2].strip(), output_folder.strip(), tmp_folder='/home/public/rmt/heren/wl-vt/tool/temp')
